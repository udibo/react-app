...
/**
 * Options for rendering the React application to a readable stream.
 */
interface RenderOptions {
  ...
  route: RouteObject;
  /**
   * The React Router static handler used to perform data fetching and form submissions on the server.
   */
  handler: ReturnType<typeof createStaticHandler>;
  /**
   * Any error that occurred during request handling.
   * This will be serialized and sent to the browser where it can be caught by an ErrorBoundary.
   * If a server error is not being caught, verify that the boundary matches the ErrorBoundary you expect to catch it.
   */
  error?: HttpError<{ boundary?: string }>;
  /** The port for the development server's live reload functionality. */
  devPort?: number;
}
 ...
 * This includes the initial state that will be sent to the client
 * and methods for rendering the application.
 */
...
> {
  /**
   * The initial state that will be serialized and sent to the browser.
   * This state can be accessed in the React application using the `useInitialState` hook.
   */
  initialState: SharedState;
  /**
   * A function that renders the application to a readable stream and sends it as the response.
   * This should be called after all necessary data has been added to the initial state.
   */
  render: () => Promise<void>;
}
